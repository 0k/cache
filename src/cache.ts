...
if (import.meta.vitest) {
    ...
    describe.each([
        ['JsonKey', JsonKeyCacheStore],
        ['TimeoutJsonKeyTTL', TimeoutJsonKeyTTLCacheStore],
        ['TimestampJsonKeyTTL', TimestampJsonKeyTTLCacheStore],
    ])('%s', (_label, cacheStore) => {

        const cache = Kache({
            cacheStore,
            ttl: 2, // only used by TTLCacheStore
        })
            ...
            describe('values', () => {
                it('should cache value like undefined', () => {
                    class A {
                        ...
                        getX (obj: any) {
                            console.warn(
                                `computing... ${JSON.stringify(obj)}['x']`,
                            )
                            return obj.x
                        }
                    }
                ...
                it('should work when using basic plain old java object', () => {
                    ...
                    const a = new A()
                    expect(
                        a.stringify([null, true, {}, ['a'], undefined]),
                    ).toBe('[null,true,{},["a"],null]')
                    expect(
                        a.stringify([null, true, {}, ['a'], undefined]),
                    ).toBe('[null,true,{},["a"],null]')
        ...
        describe('cache factory', () => {
            ...
            it('cache is its own factory 2', () => {
                const mycache = cache({ key: ({ _i, args }) => args[0] })

                class A {
                    @mycache({ cacheStore: JsonKeyCacheStore })
                    compute2x (x: number, y: number) {
                        console.warn(`computing... ${x}+${y}`)
                        return x + y
                    }
        ...
        describe('promises', () => {
            ...
            it('promises are smart when noCacheOnReject is provided', async () => {
                ...
                // the promise2 should now try the calculation
                expect(warnSpy).toHaveBeenCalledTimes(2)
                expect(warnSpy).toHaveBeenNthCalledWith(
                    2,
    ...
    describe.each([
        ...
        it('should cache values with ttl', async () => {
            ...
            const ttlcache = Kache({ cacheStore })
